// Service-job order end-to-end flow: add a part, add labor, assign a mechanic,
// save as job order, reload into a fresh cart, check out, and assert the sale
...
void main() {
  ...
  testWidgets(
      'service jobOrder round-trips labor + mechanic, then checks out '
      'with a real parts-only sales summary', (tester) async {
    // 1. Build a service job order in the cart: one part + two labor lines + mechanic.
    ...
    // 2. Save as job order.
    final jobOrder = cart.toJobOrder(
      name: 'Plate ABC-123',
      createdBy: 'cashier-1',
      createdByName: 'Cashier',
    );
    expect(jobOrder.laborLines.length, 2);
    expect(jobOrder.mechanicId, 'mech-1');
    expect(jobOrder.mechanicName, 'Juan Dela Cruz');
    expect(jobOrder.laborSubtotal, 750);
    expect(jobOrder.grandTotal, 850);

    // 3. Reload the job order into a FRESH cart — labor + mechanic must survive.
    final reloaded = CartNotifier();
    reloaded.loadFromJobOrder(jobOrder);
