...
~174px on a 924px viewport and the part "../design_handoff_closing_history/MAKI-POS-Handoff"names overlap the price column. Below ~1000px, drop
...
Placeholder: `Search part "../design_handoff_closing_history/MAKI-POS-Handoff"name or SKU — scan barcode`.

Result count below: 12px mono `--text-3`, e.g. `8 parts` / `1 part"../design_handoff_closing_history/MAKI-POS-Handoff"`.
...
`--accent-text`; `> 5` → `--surface-3` / `--text-2`. Adding an out-of-stock part "../design_handoff_closing_history/MAKI-POS-Handoff"is refused
with an error toast, not a disabled row — the clerk still needs to see it exists.

The `CopyButton` must `stopPropagation()` or copying a SKU also adds the part "../design_handoff_closing_history/MAKI-POS-Handoff"to the cart.
...
Adding a part "../design_handoff_closing_history/MAKI-POS-Handoff"already in the cart increments its qty rather than appending a second line.
...
### Empty states
Empty cart: `Cart is empty` (13px `--text-2`) + `Search a part "../design_handoff_closing_history/MAKI-POS-Handoff"and press Add to start a sale.`
...
- **Stock is decremented server-side in the same transaction.** If a part "../design_handoff_closing_history/MAKI-POS-Handoff"sold out between
