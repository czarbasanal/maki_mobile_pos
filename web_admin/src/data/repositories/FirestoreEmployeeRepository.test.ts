// Covers FirestoreEmployeeRepository.delete() only. There's no Firestore
...
const { FirestoreEmployeeRepository } = await import('./FirestoreEmployeeRepository');

describe('FirestoreEmployeeRepository.delete', () => {
  beforeEach(() => {
    state.deletes = [];
  });

  it('deletes the employee doc by id', async () => {
    const repo = new FirestoreEmployeeRepository({} as unknown as Firestore);

    await repo.delete('e1');

    expect(state.deletes).toContain('employees/e1');
  });
});
