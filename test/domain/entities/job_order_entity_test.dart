...
void main() {
  group('JobOrderEntity', () {
    late JobOrderEntity jobOrder;
    ...
    test('addItem adds new item', () {
      ...
      final updated = jobOrder.addItem(newItem);
    ...
    test('addItem updates quantity for existing product', () {
      ...
      final updated = jobOrder.addItem(sameProduct);
    ...
    test('canCheckout returns correctly', () {
      expect(jobOrder.canCheckout, true);

      final empty = jobOrder.clearItems();
      expect(empty.canCheckout, false);

      final converted = jobOrder.markAsConverted('sale-1');
    ...
    test('canCheckout false when converted even with billable content', () {
      ...
      // labor OR fees), not items alone — but a converted job order must still
      // be blocked regardless of what content it carries.
      final laborOnly = jobOrder.clearItems().addLaborLine(
            const LaborLineEntity(
                id: 'lab-1', description: 'Tune-up', fee: 300),
          );
  ...
  group('JobOrderEntity labor + money math', () {
    late JobOrderEntity jobOrder;
    ...
    test('laborLines defaults to empty and mechanic fields default to null',
        () {
      expect(jobOrder.laborLines, isEmpty);
      expect(jobOrder.mechanicId, isNull);
      expect(jobOrder.mechanicName, isNull);
    });

    test('parts getters with no labor', () {
      // subtotal: 100 * 2 = 200; discount: 10
      expect(jobOrder.partsSubtotal, 200.0);
      expect(jobOrder.laborSubtotal, 0.0);
      expect(jobOrder.partsRevenue, 190.0); // 200 - 10
      expect(jobOrder.laborRevenue, 0.0);
      expect(jobOrder.grandTotal, 190.0); // partsRevenue + laborRevenue
      // cost: 60 * 2 = 120
      expect(jobOrder.totalCost, 120.0);
      expect(jobOrder.partsProfit, 70.0); // 190 - 120
      expect(jobOrder.laborProfit, 0.0);
      expect(jobOrder.totalProfit, 70.0);
    });

    test('addLaborLine adds a labor line and feeds laborSubtotal', () {
      final updated = jobOrder.addLaborLine(
    ...
    test(
        'labor lines raise revenue/profit/grandTotal but not parts/discount/cost',
        () {
      final updated = jobOrder
          .addLaborLine(
            const LaborLineEntity(
                id: 'lab-1', description: 'Tune-up', fee: 300.0),
          )
          .addLaborLine(
            const LaborLineEntity(
                id: 'lab-2', description: 'Bleed', fee: 150.0),
          );
    ...
    test('updateLaborLine replaces a matching line by id', () {
      final updated = jobOrder
          ...
          .updateLaborLine(
            const LaborLineEntity(
                id: 'lab-1', description: 'Tune-up', fee: 500.0),
          );
    ...
    test('addFeeLine adds a fee line and feeds feesTotal', () {
      final updated = jobOrder.addFeeLine(
      ...
      // Original is unchanged (immutability).
      expect(jobOrder.feeLines, isEmpty);
    });
    ...
    test('updateFeeLine replaces a matching line by id', () {
      final updated = jobOrder
          .addFeeLine(
            const FeeLineEntity(
                id: 'fee-1', name: 'Electric charge', amount: 20.0),
          )
          .updateFeeLine(
            const FeeLineEntity(
                id: 'fee-1', name: 'Electric charge', amount: 35.0),
          );
    ...
    test('copyWith sets and clears mechanic fields', () {
      final assigned = jobOrder.copyWith(
          mechanicId: 'mech-1', mechanicName: 'Juan Dela Cruz');
    ...
    test('props include laborLines and mechanic fields', () {
      final a = jobOrder.copyWith(mechanicId: 'mech-1', mechanicName: 'Juan');
      final b = jobOrder.copyWith(mechanicId: 'mech-2', mechanicName: 'Pedro');
      expect(a == b, false);

      final withLabor = jobOrder.addLaborLine(
        const LaborLineEntity(id: 'lab-1', description: 'Tune-up', fee: 300.0),
      );
      expect(withLabor == jobOrder, false);
    });

    test('fee fields default to empty', () {
      expect(jobOrder.feeLines, isEmpty);
      expect(jobOrder.feesTotal, 0.0);
    });

    test('fees raise revenue/grandTotal but not parts/labor/discount/cost', () {
      final withFees = jobOrder.copyWith(
        feeLines: const [
    ...
    test('props include feeLines', () {
      ...
      expect(withFees == jobOrder, false);
    });
  });

  group('JobOrderEntity hasBillableContent', () {
    JobOrderEntity base() => JobOrderEntity(
          id: 'jobOrder-1',
          name: 'Table 5',
          items: const [],
          createdBy: 'cashier-1',
  ...
  group('JobOrderEntity motorcycleModel', () {
    test('copyWith + props carry motorcycleModel', () {
      final d = JobOrderEntity(
        id: 'd1',
        name: 'ABC-123',
        items: const [],
        createdBy: 'u1',
