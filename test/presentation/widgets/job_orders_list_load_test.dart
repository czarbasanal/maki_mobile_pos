...
void main() {
  ...
  JobOrderEntity jobOrder() => JobOrderEntity(
        id: 'd-1',
        name: 'Plate ABC-123',
        items: const [
          SaleItemEntity(
  ...
  Future<void> pump(WidgetTester tester) async {
    final router = GoRouter(
      initialLocation: '/job-orders',
      routes: [
        GoRoute(
            path: '/job-orders',
            builder: (_, __) => const JobOrdersListScreen()),
        GoRoute(
          path: '/job-orders/:id',
      ...
      ProviderScope(
        overrides: [
          activeJobOrdersProvider
              .overrideWith((ref) => Stream.value([jobOrder()])),
  ...
  testWidgets('app-bar plus opens the New Job Order dialog', (tester) async {
    ...
    // Read-only number derived from today's job orders (empty repo → -001);
