...
void main() {
  ...
  JobOrderEntity buildJobOrder({String? model, DateTime? updatedAt}) =>
      JobOrderEntity(
        id: 'jobOrder-1',
        name: 'Plate ABC-123',
        items: const [
  ...
  Future<void> pump(WidgetTester tester, JobOrderEntity jobOrder) async {
    ...
    await tester.pumpWidget(harness(jobOrder));
  ...
  testWidgets('info header shows the motorcycle model in the picker when set',
      (tester) async {
    await pump(tester, buildJobOrder(model: 'Yamaha Nmax'));
  ...
  testWidgets('Updated line uses the square-pen glyph', (tester) async {
    await pump(tester, buildJobOrder(updatedAt: DateTime(2026, 6, 1)));
