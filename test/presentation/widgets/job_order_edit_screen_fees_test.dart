...
void main() {
  ...
  testWidgets('adding a fee via the picker persists it onto the jobOrder',
      (tester) async {
    final (container, jobOrderId) = await pump(
      tester,
      buildJobOrder(),
    ...
    final stored = await container
        .read(jobOrderRepositoryProvider)
  ...
  testWidgets(
    ...
    final (container, _) = await pump(
      tester,
      buildJobOrder(motorcycleModel: 'Nmax'),
    );
  ...
  testWidgets('Bill out is enabled for a labor+fee ticket with no items',
      (tester) async {
  ...
  testWidgets('labor-only register cart still triggers the clobber warning',
      (tester) async {
    final (container, _) = await pump(
      tester,
      buildJobOrder(motorcycleModel: 'Nmax'),
