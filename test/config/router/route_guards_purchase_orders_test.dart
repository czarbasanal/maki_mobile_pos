...
void main() {
  ...
  test('list route: staff and admin yes, cashier no', () {
    for (final role in [UserRole.admin, UserRole.staff]) {
      expect(
          RouteGuards.canAccess(RoutePaths.purchaseOrders, user(role)), isTrue,
          reason: '$role');
    }
    expect(
        RouteGuards.canAccess(
            RoutePaths.purchaseOrders, user(UserRole.cashier)),
        isFalse);
  });
  ...
  test('dynamic child routes are gated the same', () {
    expect(
        ...
        isTrue);
    expect(
        RouteGuards.canAccess('/reorder/abc123', user(UserRole.staff)), isTrue);
  ...
  test('Reorder menu item sits after Inventory for staff/admin only', () {
    for (final role in [UserRole.staff, UserRole.admin]) {
      final paths = RouteGuards.getMenuItems(role).map((e) => e.path).toList();
    ...
    expect(
        RouteGuards.getMenuItems(UserRole.cashier).map((e) => e.path).toList(),
