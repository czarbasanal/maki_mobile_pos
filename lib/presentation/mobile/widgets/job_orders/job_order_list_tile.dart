...
/// Displays a single job order in the list, on the elevated theme.
///
/// Neutral-by-default: every job order reads with the same muted `clipboard-list`
...
class JobOrderListTile extends StatelessWidget {
  final JobOrderEntity jobOrder;
  final VoidCallback onTap;
  final VoidCallback onLoadTap;

  /// Null when the current user lacks permission to delete this job order.
  final VoidCallback? onDeleteTap;

  const JobOrderListTile({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final updatedAt = jobOrder.updatedAt ?? jobOrder.createdAt;
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    jobOrder.totalItemCount == 1
                        ? '1 item'
                        : '${jobOrder.totalItemCount} items',
                    style: TextStyle(fontSize: 12, color: muted),
                  ),
                ],
              ),
  ...
  Widget _buildItemsPreview(BuildContext context) {
    ...
    final previewItems = jobOrder.items.take(3).toList();
    final remainingCount = jobOrder.items.length - previewItems.length;
      ...
      child: Column(
        ...
        children: [
          if (jobOrder.laborLines.isNotEmpty ||
              (jobOrder.motorcycleModel?.isNotEmpty ?? false))
            Padding(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  if (jobOrder.motorcycleModel?.isNotEmpty ?? false)
                    _chip(
                      icon: LucideIcons.bike,
                      label: jobOrder.motorcycleModel!,
                      color: muted,
